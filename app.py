...
def register_user(username, email, password, full_name, address=None, phone=None, bio=None):
    ...
    success = execute_query("""
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?)
    """, (username, email, hashed.decode('utf-8'), full_name, address, phone, bio), fetch=False)
...
def show_dashboard():
    ...
    exchanges = get_user_exchanges(st.session_state.user_id).head(5)
    
    if not exchanges.empty:
        for _, ex in exchanges.iterrows():
            with st.container():
                ...
                with col3:
                    ...
                    st.write(f"{status_color} {ex['status']}")
                st.divider()
    else:
...
def show_browse_books():
    ...
    if search_term:
        ...
        if category != "All":
            books = books[books['category_name'] == category]
        if condition != "All":
            books = books[books['book_condition'] == condition]
        ...
        for _, book in books.iterrows():
            with st.container():
                ...
                with col1:
                    ...
                    st.write(f"📚 {book['category_name']} | 📖 {book['book_condition']}")
                with col2:
                    st.write(f"**Owner:** {book['owner_name']}")
                    st.write(f"⭐ Rating: {book['reputation_score']}")
                with col3:
...
def show_my_exchanges():
    ...
    with tab1:
        pending = exchanges[exchanges['status'] == 'Pending']
        for _, ex in pending.iterrows():
            display_exchange(ex)
    
    with tab2:
        approved = exchanges[exchanges['status'] == 'Approved']
        for _, ex in approved.iterrows():
            display_exchange(ex)
    
    with tab3:
        completed = exchanges[exchanges['status'] == 'Completed']
        for _, ex in completed.iterrows():
            display_exchange(ex)
...
def display_exchange(exchange):
    ...
    with st.container():
        ...
        with col3:
            ...
            st.write(f"{status_color} {exchange['status']}")
...
def show_profile():
    ...
    if not wishlist.empty:
        for _, book in wishlist.iterrows():
            with st.container():
                col1, col2 = st.columns([3, 1])
                with col1:
